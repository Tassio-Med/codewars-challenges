...
export const stockList = (listOfArt: string[], listOfCat: string[]): string => {
  ...
  const codeAndQuantity: CodeAndQuantity = {};

   listOfCat.forEach((cat) => {
    codeAndQuantity[cat] = 0;
  });
  ...
  listOfArt.forEach((art) => {
    ...
    if (listOfCat.includes(category)) {
      codeAndQuantity[category] += parseInt(quantity);
    }
  });

  const result = listOfCat.map((cat) => `(${cat} : ${codeAndQuantity[cat]})`).join(" - ");
